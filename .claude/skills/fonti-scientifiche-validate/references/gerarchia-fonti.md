...
- **Via alternativa decisiva**: **ogni revisione Cochrane è indicizzata in PubMed con l'abstract completo**, che contiene i risultati numerici principali (RR/OR con IC, numero di studi e partecipanti) e spesso la certezza GRADE per outcome («moderate certainty evidence»). Quando `cochranelibrary.com` è bloccato o a pagamento, cerca su PubMed: `<argomento> AND "Cochrane Database Syst Rev"[Journal]`. Le SoF complete e la PLS non sono in PubMed.
